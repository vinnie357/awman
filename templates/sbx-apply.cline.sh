#!/usr/bin/env bash
# awman sbx startup script — agent: cline
...
if ! command -v jq >/dev/null 2>&1; then
  echo "awman: jq not found — skipping session config apply for cline." >&2
  exit 0
fi
...
# ---------------------------------------------------------------------------
# Agent-specific config rendering for cline is minimal.
# env_config entries are written to $HOME/.awman/env.sh (sourceable).
# ---------------------------------------------------------------------------
mkdir -p "$HOME/.awman"
ENV_SH="$HOME/.awman/env.sh"

printf '# awman — generated environment overrides for cline\n' > "$ENV_SH"
...
echo "awman: cline session config applied from $SESSION_FILE."
