...
export AMUX_OVERLAYS="skill()"
```

amux resolves the correct container path for each agent automatically — Claude Code's `~/.claude/commands/`, Codex's `~/.codex/skills/`, Gemini's `~/.gemini/commands/`, and so on. The mount is always read-only.

Skills overlay can be combined with directory overlays in a single `--overlay` flag:

```sh
amux chat --overlay "skill(),dir(/data/reference:/mnt/reference:ro)"
```

See [Security & Isolation](../04-security-and-isolation.md#skills-overlay) and [Configuration](../08-configuration.md#overlays) for full details.
